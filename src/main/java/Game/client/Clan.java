...
public class Clan {
    ...
    public void sortMember() {
        mems.sort((m1, m2) -> Byte.compare(m2.mem_type, m1.mem_type));
    }
        ...
        switch (type) {
            ...
            case 18: {
                ...
                if (get_mem_type(conn.p.name) == 127 && !conn.p.name.equals(name)) {
                    if (camroibang == true){
                        Service.send_notice_box(conn,"Đang trong khung giờ cấm không thể kick thành viên bang");
                        return;
                    }
                    ...
                    if (camroibang == true){
                        Service.send_notice_box(conn,"Đang trong khung giờ cấm không thể rời bang");
                        return;
                    }
    ...
    public void member_contribute_ngoc(Session conn, long value) throws IOException {
        ...
        if (conn.p.diem_tieu_sai < 20000) {
            Service.send_notice_box(conn, "Bạn cần ít nhất 20.000 điểm tiêu sai để sử dụng chức năng này!");
            return;
        }
    ...
    public synchronized static void end_create_clan(Session conn, short idIcon) throws IOException {
        ...
        for (int i = 0; i < conn.p.item.wear.length; i++) {
            ...
            temp2.type = (byte) conn.p.item.wear[i].type;
            temp2.part = (byte) conn.p.item.wear[i].part;
    ...
    public synchronized void accept_mem(Session conn, Player p0) throws IOException {
        ...
        if (conn.p.level < 30) {
            Service.send_notice_box(conn, "Bạn cần đạt cấp độ 30 mới có thể gia nhập bang!");
            return;
        }
    ...
    public synchronized void update_exp(int exp) {
        ...
        this.exp += exp;
        if (this.exp > Level.entry.get(this.level).exp) {
            this.exp = Level.entry.get(this.level).exp;
        }
//        if (this.get_percent_level() >= 100 && this.level != 9 && this.level != 19 && this.level != 29 && this.level != 39 && this.level != 49) {
//            this.exp = 0;
//            this.level++;
//        } else {
//            this.exp = LevelClan.entry.get(this.level - 1).exp -1;
//        }
    }

    public synchronized void update_level() {
        if (this.get_percent_level() >= 1000) {
            this.exp = 0;
            this.level++;
            if (this.level == 2 || this.level == 10 || this.level == 20 || this.level == 30) {
                if (this.max_mem < 5) {
                    this.max_mem += 1;
                }
            }
