...
public class Squire extends Player {
    ...
    public ArrayList<Byte> get_skill_can_use() {
        ...
        byte[] idSkill = {0, 1, 2, 3, 4, 5, 6, 7, 8, 17, 19, 20};

        for (byte id : idSkill) {
            if (id >= 0 && id < skill_point.length && skill_point[id] > 0) {
                skills.add(id);
            }
        }
        return skills;
    ...
    public void use_skill(Map map, Session conn, int n, int ObjAtk, short n3, int type_atk) {
        try {
            ArrayList<Byte> idSkill = get_skill_can_use();
            if (idSkill.isEmpty()) {
                return; // hoặc xử lý không có skill
            }

            long time_ = System.currentTimeMillis();
            byte index_skill = idSkill.get(Util.random(idSkill.size()));

            int loop = 0;
            while ((time_ < this.time_delay_skill[index_skill]
                    || this.skill_point[index_skill] < 1)
                    && loop < idSkill.size()) {

                index_skill = idSkill.get(Util.random(idSkill.size()));
                loop++;
            }

            if (time_ < this.time_delay_skill[index_skill]
                    || this.skill_point[index_skill] < 1) {
                return; // không có skill nào hợp lệ
            }
