...
public class Item {
    ...
    public void add_item_inventory47(Item47 item) {
    if (inventory47 == null) {
            inventory47 = new ArrayList<>();
        }
        inventory47.add(item); // Thêm vật phẩm vào danh sách
    }
    ...
    public void add_item_inventory3_default(short it, int date, boolean isLock) {
        ...
        itbag.icon = ItemTemplate3.item.get(it).getIcon();
        itbag.op = new ArrayList<>();
        for (Option o : ItemTemplate3.item.get(it).getOp()) {
    ...
    public void remove_item_ticket(int id, int type, int quantity) {
        int q = quantity;
        for (int i = 0; i < inventory3.length; i++) {
            Item3 it = inventory3[i];
            if (it == null)
