...
//                }
                ...
                case -112: {
                    if (conn.p.get_ngoc() < Map.NGOC_KHU_2) {
                    ...
                    conn.p.update_ngoc(-Map.NGOC_KHU_2);
                ...
                case 12: {
                    ...
                    if (item != null) {
                        List<WearEffect.EffectInfo> listInfo = WearEffect.getListEffect(item.id);
                        if (listInfo != null) {
                            for (WearEffect.EffectInfo info : listInfo) {
                                // Gửi thời gian = 0 để Client tắt hiệu ứng NGAY LẬP TỨC
                                if (info.type == 112) {
                                    conn.p.send_eff_112(info.effId, 0);
                                } else {
                                    conn.p.send_eff_111(info.effId, 0);
                                }
                            }
                        }
                ...
                case 122: {
                    byte itemTier = conn.p.item.inventory3[conn.p.item_replace].tier;

                    if (itemTier <= 10 && conn.p.item.total_item_by_id(7, 346) < 1) {
                        Service.send_notice_box(conn, "Không đủ đá thạch anh cấp 1.");
                        return;
                    } else if (itemTier > 10 && itemTier <= 13 && conn.p.item.total_item_by_id(7, 347) < 1) {
                        Service.send_notice_box(conn, "Không đủ đá thạch anh cấp 2.");
                        return;
                    } else if (itemTier > 13 && itemTier <= 15 && conn.p.item.total_item_by_id(7, 348) < 1) {
                        Service.send_notice_box(conn, "Không đủ đá thạch anh cấp 3.");
                        return;
                    }

                    if (itemTier <= 10) {
                        conn.p.item.remove(7, 346, 1);
                    } else if (itemTier > 10 && itemTier <= 13) {
                        conn.p.item.remove(7, 347, 1);
                    } else if (itemTier > 13 && itemTier <= 15) {
                        conn.p.item.remove(7, 348, 1);
                    }

                    conn.p.item.inventory3[conn.p.item_replace2].tier = itemTier;
                ...
                case 112: {
                    try {
                        ...
                        if (temp == null) return;
                        int token = (temp.it.tier + 1) * 100;
                        // 1. Kiểm tra tiền (Phải khớp với thông báo là 10.000)
                        if (conn.p.getToken() < token) {
                            Service.send_notice_box(conn, "Cần "+token+" token để đột phá!");
                            return;
                        }
                        // 3. Tiến hành đột phá
                        temp.it.tier++;
                        Service.send_notice_box(conn, "Đột phá thành công! Giới hạn mới: " + (10 + temp.it.tier * 10) + "%");
                        conn.p.update_token(-token);
                        // Cập nhật bản thân
                        ...
                        if (p0 != null) {
                            ...
                            Service.send_char_main_in4(p0);
                            MapService.update_in4_2_other_inside(p0.map, p0);
                            Service.send_notice_box(p0.conn, "Bạn đời đã đột phá nhẫn lên cấp " + temp.it.tier);
                        }
                    } catch (Exception e){
                        e.printStackTrace();
                    }
                ...
                case 110: {
                    ...
                    p0.update_coin(-100000);
                    conn.p.update_coin(-100000);
                    conn.p.history_coin(-100000,"(TRỪ COIN) từ kết hôn");
                    p0.history_coin(-100000,"(TRỪ COIN) từ kết hôn");
...
//                    hisc.Flus();
//                    His_COIN his = new His_COIN(conn.user ,p0.name);
//                    his.coin_change = 100_000;
//                    his.coin_last = p0.getCoin();
//                    his.Logger = "(TRỪ COIN) từ kết hôn";
//                    his.Flus();
                ...
                case -11: { // Xử lý MUA QUỸ (Sửa giá thành 100k)
                    if (conn.p.da_mua_quy == 1) {
                        Service.send_notice_box(conn, "Bạn đã mua gói này rồi!");
                        return;
                    }
                    if (conn.p.getCoin() < 100000) {
                        Service.send_notice_box(conn, "Bạn cần 100.000 Coin để kích hoạt.");
                        return;
                    }

                    conn.p.update_coin(-100000); // Trừ 100k
                    conn.p.history_coin(-100000,"(TRỪ COIN) Quỹ đầu tư");
//                    His_COIN hisc = new His_COIN(conn.user ,conn.p.name);
//                    hisc.coin_change = 100_000;
//                    hisc.coin_last = conn.p.getCoin();
//                    hisc.Logger = "(TRỪ COIN) Quỹ đầu tư";
//                    hisc.Flus();
                    conn.p.da_mua_quy = 1;
                    conn.p.moc_nhan_quy = 0;

                    Service.send_notice_box(conn, "Đầu tư thành công! Hãy cày cấp để nhận lại 900k Coin.");
                    break;
                }

                case -12: { // Xử lý NHẬN THƯỞNG
                    MenuController.xuLyNhanThuongQuy(conn.p); // Gọi lại hàm nhận thưởng mình đã viết ở bài trước
                    break;
                }
            }
        }
