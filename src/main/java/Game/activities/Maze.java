...
public class Maze {
    ...
    public static final short[] den_than_106 = new short[]{500, 500};  // Vị trí Đèn Thần trên map 106
    public static final short[] den_than_107 = new short[]{600, 600};  // Vị trí Đèn Thần trên map 107

    public static void updatePlayer(Player p, Map map) {
        try {
            ...
            e.printStackTrace(); // Xử lý ngoại lệ, ví dụ ghi vào log hoặc gửi thông báo lỗi cho người chơi
        }
    }
    ...
    public static Vgo getVgo(Player p) {
        if (p != null) {
            if (p.map.map_id == 105) {
                ...
                if (isDenThan(p.x, p.y, den_than_106)) {
                    return giveTreasure(p);  // Trao phần thưởng khi gặp Đèn Thần
                }
    ...
    public static boolean isDenThan(int x, int y, short[] denThanPos) {
        // Kiểm tra khoảng cách giữa tọa độ người chơi và tọa độ Đèn Thần
        return (Math.abs(denThanPos[0] - x)) < 40 && (Math.abs(denThanPos[1] - y)) < 40;
    }

    public static Vgo giveTreasure(Player p) {
        // Danh sách các phần thưởng ngẫu nhiên khi người chơi tìm thấy Đèn Thần
        String[] rewards = {
                "Thẻ trao đổi đồng/bạc/vàng/kim cương",
                "Đồ thời trang Alladin/Jasmin",
                "Đục huyền bí cấp 1/2/3",
                "Đá 3 màu",
                "10-20 Cỏ 3 lá",
                "100~200 Bình MP 600",
                "100~200 Bình HP 6000",
                "3~5 Thánh giá",
                "3~5 Xương",
                "3~5 Đá ánh sáng cấp 1/2",
                "3~5 Đá bóng tối cấp 1/2",
                "Exp luôn có",
                "10~20 Titan cấp 1-2",
                "10~20 Kim loại vũ trụ",
                "10~20 Phù về làng",
                "1 Vé x2 đi buôn",
                "1 Sách Skill đệ tử ngẫu nhiên",
                "1 Sách tri thức",
                "100 Điểm sức khỏe",
                "5~10 thú cưỡi Heo rừng",
                "1~10 viên Đá Phân Thân",
                "1~3 Vé mở ly cánh thời trang"
        };

        // Chọn phần thưởng ngẫu nhiên từ danh sách
        String reward = rewards[Util.random(rewards.length)];


        // Trả về phần thưởng
        Vgo vgo = new Vgo();
        vgo.id_map_go = p.map.map_id;  // Đảm bảo không thay đổi bản đồ
        vgo.x_new = p.x;  // Giữ vị trí hiện tại của người chơi
        vgo.y_new = p.y;  // Giữ vị trí hiện tại của người chơi
        return vgo;
    }

    public static void sendRewardMessage(Player p, String reward) {
        try {
            Message m = new Message(27);
            m.writer().writeShort(p.ID);
            m.writer().writeByte(0);
            m.writer().writeUTF("Chúc mừng bạn đã tìm thấy Đèn Thần! Bạn nhận được: " + reward);
            p.conn.addmsg(m);
            m.cleanup();
        } catch (IOException e) {
            e.printStackTrace();
