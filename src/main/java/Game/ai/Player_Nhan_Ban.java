        ...
        for (int i = 0; i < ChienTruong.gI().list_ai.size(); i++) {
            ...
            if (temp.target == -1) {
                for (int j = 0; j < map.players.size(); j++) {
                    ...
                    if (p_target != null && p_target.typepk != temp.typePk && Math.abs(p_target.x - temp.x) < 300
                            && Math.abs(p_target.y - temp.y) < 300 && temp.time_change_target < System.currentTimeMillis()) {
            ...
            if (p0 != null && temp.typePk != p0.typepk && !p0.isdie && p0.map.equals(temp.map)) {
                // Chỉ đánh nếu còn trong tầm 300
                if (Math.abs(p0.x - temp.x) < 300 && Math.abs(p0.y - temp.y) < 300) {
                    atk(map, temp, p0);
                }
            }
            // 3. Logic DI CHUYỂN (Giữ nguyên hoặc giảm delay để linh hoạt hơn)
            if (!temp.isdie && temp.map.equals(map) && temp.time_move < System.currentTimeMillis()) {
                temp.time_move = System.currentTimeMillis() + Util.random(1500, 3000); // Giảm xuống 1.5 - 3s cho lanh lẹ
                if (p0 != null && temp.typePk != p0.typepk && !p0.isdie && p0.map.equals(temp.map)) {
                    // Nếu có mục tiêu -> Lao tới mục tiêu
                    if (Math.abs(p0.x - temp.x) < 300 && Math.abs(p0.y - temp.y) < 300) {
