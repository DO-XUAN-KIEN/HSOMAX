...
public class LuckyDraw implements Runnable {
   ...
   // System.out.println("[DEBUG] === Danh sách người chơi tham gia VXMM ===");
    for (PlayerJoin pj : listPlayer) {
    //    System.out.println(" - " + pj.name + " (ID=" + pj.ID + ", gold=" + pj.gold + ")");
    }
   // System.out.println("===============================================");

    // ƯU TIÊN ADMIN = 10
    for (int i = 0; i < listPlayer.size(); i++) {
        PlayerJoin pj = listPlayer.get(i);
        for (int j = Session.client_entry.size() - 1; j >= 0; j--) {
            Session s = Session.client_entry.get(j);
            if (s != null && s.p != null) {
               // System.out.println("[DEBUG] Kiểm tra: " + s.p.name + " (ID=" + s.p.ID + ", admin=" + s.ac_admin  + ")");
                if (s.p.ID == pj.ID && s.ac_admin > 2) {
                   // System.out.println("[DEBUG] >>> ADMIN 10 ĐƯỢC ƯU TIÊN: " + s.p.name);
                    index = i;
    ...
    // Nếu không có admin, dùng cơ chế random theo tỉ lệ
    if (index == -1) {
    ...
    // Nếu vẫn chưa chọn được, random hoàn toàn
    if (index == -1) {
        index = Util.random(0, listPlayer.size());
       // System.out.println("[DEBUG] Không có người thắng theo tỉ lệ, random ngẫu nhiên: index = " + index);
    }

    // Tiến hành trao thưởng
    dem = 0;
    for (PlayerJoin player : listPlayer) {
        if (dem == index) {
            Player p0 = null;
            ...
            long thue = (vang_win / 100L) * Manager.thue;
            long thueTOP1 = (vang_win / 100L) * 1;
            vang_win -= thue;
            vang_win -= thueTOP1;
