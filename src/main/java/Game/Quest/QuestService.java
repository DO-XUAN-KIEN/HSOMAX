  ...
  public static void sendQuestFinish(Session conn) {
    try {
        ...
        for (int i = 0; i < p.questsFinish.size(); i++) {
            ...
            if (quest != null) {
                // Kiểm tra nếu strDetailTalkFinish hoặc strDetailHelpFinish là null hoặc rỗng
                if ((quest.strDetailTalkFinish == null || quest.strDetailTalkFinish.isEmpty()) || 
                    (quest.strDetailHelpFinish == null || quest.strDetailHelpFinish.isEmpty())) {
                    continue; // Nếu rỗng hoặc null, bỏ qua nhiệm vụ này
                }
