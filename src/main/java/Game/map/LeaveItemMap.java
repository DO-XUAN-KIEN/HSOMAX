...
public class LeaveItemMap {
    ...
    public static void leave_vang(Map map, Mob_in_map mob, Player p) throws IOException {
        if (mob != null) {
            ...
            if (index_item_map > -1) {
                ...
                map.item_map[index_item_map].setIdmaster((short) p.ID);
                map.item_map[index_item_map].time_exist = System.currentTimeMillis() + 200_000L;
   ...
   public static void leave_item_3(Map map, Mob_in_map mob, Player p) throws IOException {
    if (mob != null) {
        ...
        // Kiểm tra điều kiện drop item
        if (p.isDropItemColor4 && color_ != 4) {
            return;
        }

        // Chọn item theo level mob
        ...
        // Kiểm tra nếu item tồn tại trong ItemTemplate3
        if (id_item_can_drop > 0 && id_item_can_drop < ItemTemplate3.item.size()) {
            String name = ItemTemplate3.item.get(id_item_can_drop).getName();
            short index_real = 0;

            // Tìm index thực của item với màu sắc tương ứng
            ...
            // Gọi hàm leave_item_by_type3 nếu tìm thấy item hợp lệ
            if (index_real > 0) {
   ...
   public static void leave_item_by_type3(Map map, int index_real, int color_, Player p_master, String name, int index) throws IOException {
    // Lấy chỉ số item_map có thể sử dụng
    int index_item_map = map.get_item_map_index_able();

    // Kiểm tra nếu index_item_map hợp lệ
    if (index_item_map > -1) {
        // Nếu item_map[index_item_map] là null, khởi tạo đối tượng ItemMap
        if (map.item_map[index_item_map] == null) {
            map.item_map[index_item_map] = new ItemMap();
        }

        // Gán các giá trị vào đối tượng item_map[index_item_map]
        ...
        map.item_map[index_item_map].setIdmaster((short) p_master.ID);

        // Khởi tạo danh sách Option mới
        ...
        // Nếu không tìm thấy chỉ số hợp lệ
...
public static void leave_item_4(Map map, Mob_in_map mob, Player p) throws IOException {
    if (mob != null) {
        if (p.allowDropNL) {
            // Rơi NL đặc biệt 100%
            short[] ids = new short[]{38, 39, 40, 41, 42, 43, 105, 106, 107, 108, 109, 110};
            short index_real = ids[Util.random(0, ids.length)];
            leave_item_by_type4(map, index_real, p, mob.ID);
        } else {
            // Rơi mặc định nếu chưa đăng ký
  ...
  public static void leave_item_7(Map map, Mob_in_map mob, Player p) throws IOException {
    if (mob != null) {
        ...
        if (Util.random(1000000) < 1 ) { 
            short[] ngocRong = {464, 465, 466, 467, 468, 469, 470};
            short id_ngoc = ngocRong[Util.random(ngocRong.length)];
            leave_item_by_type7(map, id_ngoc, p, mob.ID, p.ID);
        }

        
        if (Util.random(100) < 1) { // 
            short[] newItems = {23, 28};
            short id_new = newItems[Util.random(newItems.length)];
            leave_item_by_type7(map, id_new, p, mob.ID, p.ID);
        }
        ...
        // Rơi item bình thường
        short index_real = (short) Util.random(0, 4);
        leave_item_by_type7(map, index_real, p, mob.ID, p.ID);
    }
}
    ...
    public static void leave_item_by_type7(Map map, short id_it, Player p_master, int indexmob) throws IOException {
        ...
        if (index_item_map > -1) {
            ...
            map.item_map[index_item_map].setIdmaster((short) p_master.ID);
            map.item_map[index_item_map].time_exist = System.currentTimeMillis() + 180_000L;
            map.item_map[index_item_map].time_pick = System.currentTimeMillis() + 1_500L;
            // add in4 game scr
            Message mi = new Message(19);
    ...
    if (mob != null && mob.isBoss()) {
        // mảng rơi đồ tạm thời
        short[] id_item_leave3 = new short[]{};
        short[] id_item_leave4 = new short[]{};
        short[] id_item_leave7 = new short[]{};
        short sizeRandomMedal = 50;

        switch (mob.template.mob_id) {
            case 101: { // xa nu
                id_item_leave4 = new short[]{-1, -1, -1, -1, -1, -1, 53};
                id_item_leave7 = new short[]{0, 4, 13, 2, 1, 10, 8, 9 , 10, 11, 458, 458, 14};
                break;
            }
            case 84: { // de vang
                id_item_leave4 = new short[]{-1, -1, -1, -1, -1, -1, 54};
                id_item_leave7 = new short[]{0, 4, 13, 2, 1, 10, 8, 9 , 10, 11, 459, 459, 14};
                break;
            }
            case 83: { // de bac
                id_item_leave4 = new short[]{-1, -1, -1, -1, -1, -1, 53};
                id_item_leave7 = new short[]{0, 4, 13, 2, 1, 10, 8, 9 , 10, 11, 460, 462, 460, 462, 14};
                break;
            }
            case 103, 196, 186, 187, 188 ,197: { // bo cap chua
                id_item_leave4 = new short[]{-1, -1, -1, -1, -1, -1, 54};
                id_item_leave7 = new short[]{0, 4, 13, 2, 1, 10, 8, 9 , 10, 11, 14};
                break;
            }
            case 104: { // quy 1 mat
                id_item_leave4 = new short[]{-1, -1, -1, -1, -1, -1, 53};
                id_item_leave7 = new short[]{0, 4, 13, 2, 1, 10, 8, 9 , 10, 11, 460, 460, 14};
                break;
            }
            case 105: { // quy dau bo
                id_item_leave4 = new short[]{-1, -1, -1, -1, -1, -1, 54};
                id_item_leave7 = new short[]{0, 4, 13, 2, 1, 10, 8, 9 , 10, 11, 457, 457, 14};
                break;
            }
            case 106: { // ky sy dia nguc
                id_item_leave4 = new short[]{-1, -1, -1, -1, -1, -1, 54};
                id_item_leave7 = new short[]{0, 4, 13, 2, 1, 10, 8, 9 , 10, 11, 461, 461, 14};
                break;
            }
            case 149: { // nhen chua
                id_item_leave4 = new short[]{-1, -1, -1, -1, -1, -1, 54};
                id_item_leave7 = new short[]{0, 4, 13, 2, 1, 10, 8, 9 , 10, 11, 462, 462, 14};
                break;
            }
            case 155: { // giant skeleton
                id_item_leave4 = new short[]{-1, -1, -1, -1, -1, -1, 54};
                id_item_leave7 = new short[]{0, 4, 13, 2, 1, 10, 8, 9 , 10, 11, 463, 463, 14};
                break;
            }
            case 173: { // tho tuyet
            ...
            case 174: { // Boss Sự Kiện
                id_item_leave4 = new short[]{
                    -1, -1, -1, -1, -1, -1,
                    184, 185, 186, 187, 188, 189, 190, 191, 52, 259, 205, 206, 207
                };
        ...
        for (short id : id_item_leave3) {
            ...
            if (Item3.isBook(temp.getId())) temp.setColor((byte) 5);
        ...
        // Spawn Item7 cũ
        for (int i = 0; i < 100; i++) {
            if (id_item_leave7.length == 0) break;
            leave_item_by_type7(map, id_item_leave7[Util.random(id_item_leave7.length)], p, mob.ID, p.ID);
        }

        // **Spawn thêm 1 viên Item7 ID 493 cho tất cả boss**
        leave_item_by_type7(map, (short) 493, p, mob.ID, p.ID);
    }
}
    ...
    public static void leave_vang(Map map, Mob_in_map mob, int id_mater) throws IOException {
        if (mob != null) {
            ...
            if (index_item_map > -1) {
                ...
                map.item_map[index_item_map].quantity = vang_drop;
                map.item_map[index_item_map].category = 4;
                map.item_map[index_item_map].setIdmaster((short) id_mater);
                map.item_map[index_item_map].time_exist = System.currentTimeMillis() + 200_000L;
                map.item_map[index_item_map].time_pick = System.currentTimeMillis() + 1_500L;
                String name = "vàng *" + map.item_map[index_item_map].quantity;
                // add in4 game scr
    ...
    public static void leave_item_by_type3(Map map, int index_real, int color_, Player p_master, String name, int index, int idP)
        ...
        if (index_item_map > -1) {
            ...
            map.item_map[index_item_map].quantity = 1;
            map.item_map[index_item_map].category = 3;
            map.item_map[index_item_map].setIdmaster((short) idP);
    ...
    public static void leave_item_by_type7(Map map, short id_it, Player p_master, int index, int idp) throws IOException {
        ...
        if (index_item_map > -1) {
            ...
            map.item_map[index_item_map].category = 7;
            map.item_map[index_item_map].setIdmaster((short) idp);
            map.item_map[index_item_map].time_exist = System.currentTimeMillis() + 80_000L;
            map.item_map[index_item_map].time_pick = System.currentTimeMillis() + 1_500L;
