...
public class Item3 {
    ...
    public short id;
    public short clazz;   // ← đổi
    public short type;    // ← đổi
    public short level;
    public short icon;
    public short color;   // ← đổi (nếu có thể >127)
    public short part;    // ← đổi
...
temp.name  = ItemTemplate3.item.get(temp.id).getName();
  ...
  public boolean isTT() {
    ...
    public boolean isTrangBi() {
        return this.type >= 0 && this.type <= 4 || this.type >= 6 && this.type <= 16;
    }
