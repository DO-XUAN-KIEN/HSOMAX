...
public class Manager {
    ...
    public ArrayList<Game.io.Session> conns = new ArrayList<>();
    ...
    // Thêm vào class Manager
    public static long time_discount_server = 0; // Thời gian kết thúc sự kiện giảm giá
    public static int percent_discount = 0;      // % giảm giá (Ví dụ: 10, 20...)
    ...
    public static void setMaintenance(boolean on, String reason) {
        maintenance = on;
...
//
//            String[] thongBao = {
//                    "🔥 Thần long giáo khai mở!",
//                    "🌟 Code: tanthu , code hằng ngày, ( ngày + tháng , ví dụ 0712 )!",
//                    "🌟 Kích hoạt tài khoản  tại NPC ADMIN!",
//                    "💎 Nếu gặp lỗi trong quá trình chơi, hãy báo ngay cho ADMIN để được hỗ trợ!",
//                    "⚠️ Lợi dụng lỗi hoặc bug để chơi game sẽ bị BAN vĩnh viễn!",
...
//                while (true) {
//                    try {
//                        for (String msg : thongBao) {
//                            Manager.gI().chatKTGprocess(msg);
//                            Thread.sleep(10000); // nghỉ 5 giây giữa mỗi thông báo
//                        }
//                        Thread.sleep(5 * 60 * 1000); // nghỉ 30 phút mới chạy lại bộ 4
//                    } catch (Exception e) {
//                        e.printStackTrace();
//                    }
//                }
//            }).start();
    ...
    private boolean load_database() {
        try {
            ...
            while (rs.next()) {
                ...
                temp.setType(rs.getShort("type"));     // ✅
                temp.setPart(rs.getShort("part"));     // ✅
                temp.setClazz(rs.getShort("clazz"));   // ✅
                temp.setIcon(rs.getShort("iconid"));
                temp.setLevel(rs.getShort("level"));
                temp.setColor(rs.getShort("color"));   // ✅
                    ...
                    case 0: {
                        switch (this.event) {
                            ...
                            case 4: {
                                ...
                                itemPoitionSell[itemPoitionSell.length - 2] = 140;
                                itemPoitionSell[itemPoitionSell.length - 1] = 141;
                                item_sell.get(Service.SHOP_POTION).add((short) 140);
                                item_sell.get(Service.SHOP_POTION).add((short) 141);
                                break;
                            }
                            default: {
                    ...
                    case 16: {
                        ...
                        if (Manager.gI().event == 2 && type >= 1 && type <= 4) {
                            ...
                            for (int i = 0; i < id_.length; i++) {
                                ...
                                itemsell3[i + jsar.size()].clazz = (byte) temp.getClazz();
                                itemsell3[i + jsar.size()].type = (byte) temp.getType();
                                itemsell3[i + jsar.size()].price = 50;
                                itemsell3[i + jsar.size()].level = temp.getLevel();
                                itemsell3[i + jsar.size()].color = (byte) temp.getColor();
            ...
            ps.close();
            conn.close();
            ShopCustom.loadShopGems();
            ShopCustom_Moi.loadShopGems();
            WearEffect.loadWearEffects();
            add_item_sell_other();
            return true;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void load_config() throws IOException {
        ...
        this.mysql_pass = configMap.getOrDefault("mysql-password", "");
        this.mysql_database = configMap.getOrDefault("mysql-database", "maxhso");
    ...
    public void chatKTGWhite(String s) throws IOException {
        Message m = new Message(53);
        m.writer().writeUTF(s);
        m.writer().writeByte(0); // KTG trắng
    ...
    // Hàm tìm người chơi đang online bằng ID
    public Player get_player(int id) {
        // Duyệt qua danh sách các kết nối đang online
        for (Session conn : Session.client_entry) {
            if (conn != null && conn.p != null && conn.p.ID == id) {
                return conn.p;
            }
        }
        return null; // Không tìm thấy hoặc đang offline
    }
    // --- CODE CỘNG VÀNG TOP 1 (Đã fix theo source HSO của bạn) ---
    public static int CACHE_ID_TOP_1 = -1;
    public static long LAST_TIME_CHECK = 0;

    public static void congVangChoTop1(long vangThue) {
        if (vangThue <= 0) return;

        // --- PHẦN 1: TÌM ID NHÂN VẬT CỦA TOP 1 NẠP ---
        // Kiểm tra cache để tránh truy vấn liên tục (5 phút check 1 lần)
        long now = System.currentTimeMillis();

        // Nếu chưa có Cache hoặc Cache đã cũ quá 5 phút
        if (CACHE_ID_TOP_1 == -1 || now - LAST_TIME_CHECK > 300000) {
            Connection conn = null;
            ...
            try {
                ...
                // BƯỚC 1: Tìm tài khoản Top nạp và lấy danh sách tên nhân vật (cột char)
                // Lệnh SQL này chỉ lấy cột 'char' từ bảng 'account' sắp xếp theo 'tongnap'
                String sqlGetAccount = "SELECT `char` FROM `account` WHERE `tongnap` > 0 ORDER BY `tongnap` DESC LIMIT 1";
                ps = conn.prepareStatement(sqlGetAccount);
                rs = ps.executeQuery();

                String jsonCharList = "";
                if (rs.next()) {
                    jsonCharList = rs.getString("char"); // Kết quả sẽ có dạng: ["TênNV"]
                }
                // Đóng ResultSet và PreparedStatement cũ để tái sử dụng
                rs.close();
                ps.close();

                // BƯỚC 2: Xử lý chuỗi JSON để lấy ra tên nhân vật sạch
                String cleanName = null;
                // Kiểm tra xem chuỗi có dữ liệu không (độ dài > 2 vì ít nhất phải là "[]")
                if (jsonCharList != null && jsonCharList.length() > 2) {
                    // Dùng Regex xóa hết các ký tự [ ] " để lấy tên thuần
                    // Ví dụ: ["gamoi"] --> gamoi
                    cleanName = jsonCharList.replaceAll("[\"\\[\\]]", "").trim();

                    // Trường hợp 1 tài khoản có nhiều nhân vật: ["nv1", "nv2"] --> nv1, nv2
                    // Ta sẽ lấy tên nhân vật đầu tiên trong danh sách (thường là nhân vật chính)
                    if (cleanName.contains(",")) {
                        cleanName = cleanName.split(",")[0];
                    }
                }

                // BƯỚC 3: Có tên rồi, sang bảng 'player' tìm ID của nhân vật đó
                if (cleanName != null && !cleanName.isEmpty()) {
                    String sqlGetPlayerID = "SELECT `id` FROM `player` WHERE `name` = ?";
                    ps = conn.prepareStatement(sqlGetPlayerID);
                    ps.setString(1, cleanName); // Gán tên vừa lọc được vào dấu ?
                    rs = ps.executeQuery();

                    if (rs.next()) {
                        CACHE_ID_TOP_1 = rs.getInt("id"); // Đây mới là ID nhân vật chính xác để cộng tiền
                    } else {
                        // Trường hợp có tên trong account nhưng đã bị xóa khỏi bảng player
                        CACHE_ID_TOP_1 = -1;
                    }
                } else {
                    CACHE_ID_TOP_1 = -1;
        ...
        // Nếu không tìm thấy ai (Server mới mở hoặc lỗi) thì dừng
        if (CACHE_ID_TOP_1 == -1) return;

        // --- PHẦN 2: CỘNG TIỀN CHO ID VỪA TÌM ĐƯỢC ---
        // Kiểm tra xem nhân vật có đang Online không
        Player pTop = Manager.gI().get_player(CACHE_ID_TOP_1);

        if (pTop != null && pTop.conn != null) {
            // [TRƯỜNG HỢP ONLINE]
            pTop.update_vang(vangThue, "Thuế Top 1 Nạp");
            try {
                Service.send_notice_nobox_white(pTop.conn, "Chúc mừng! Bạn nhận được " + Util.number_format(vangThue) + " vàng thuế từ server!");
            } catch(Exception e) {}
        } else {
            // [TRƯỜNG HỢP OFFLINE] -> Cộng trực tiếp vào Database
            Connection conn = null;
            PreparedStatement ps = null;
            try {
                conn = SQL.gI().getConnection();
                // Lưu ý: Lúc này ta update bảng 'player' dựa trên 'id' (id nhân vật)
                String sqlUpdate = "UPDATE `player` SET `vang` = `vang` + ? WHERE `id` = ?";
                ps = conn.prepareStatement(sqlUpdate);
                ps.setLong(1, vangThue);
                ps.setInt(2, CACHE_ID_TOP_1);
                ps.executeUpdate();
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
