...
public class Util {
    ...
    private static final NumberFormat en = NumberFormat.getInstance(new Locale("vi"));
     private static final SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
           ...
           private static final ScheduledExecutorService SCHED = Executors.newScheduledThreadPool(4);

    public static ScheduledFuture<?> everyFixed(long initialDelay, long period, TimeUnit unit, Runnable task) {
        return SCHED.scheduleAtFixedRate(task, initialDelay, period, unit);
    }

    public static ScheduledFuture<?> after(long delay, TimeUnit unit, Runnable task) {
